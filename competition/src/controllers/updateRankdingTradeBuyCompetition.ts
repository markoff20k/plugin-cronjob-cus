...
import {
    createRankCompetition, getCompetitionActive, getListMemberBuyTrade, getMemberByMemberID, getTradeValid, isExistRank, updateNextDateCompetitionByID, updateRank
} from "../services";


export const updateRankTradeBuyCompetition = async () => {
    console.log(">>>>>>>>> : Update Trading Competitions Ranks Running");
    try {
        const cronjob_time = 30;
        const competitionsActive = await getCompetitionActive("trade_buy");
        for (
            ...
            const listMember = getListMemberBuyTrade(
                tradesValid,
                toNumber(min_value)
            );
